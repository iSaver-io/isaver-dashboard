...
import { RaffleAdditionalData } from '@/hooks/lottery/useLotteryRoundAdditionalData';
import { Helper, ILottery } from '@/types.common';

import { getLotteryTitle } from '../lottery';
...
export const parseLotteryFormat = (
  round: ILottery.RoundStructOutput,
  ...
  return {
    ...round,
    id,
    title: raffleDataMap[id] ? `${raffleDataMap[id].title} ${id + 1}` : getLotteryTitle(id + 1),
