...
import { AddTeamModal } from '@/components/AdminPanel/common/AddTeamModal';
import { AdminSection } from '@/components/AdminPanel/common/AdminSection';
import { Button } from '@/components/ui/Button/Button';
import { useTeamPlans } from '@/hooks/teams/useTeams';
import { bigNumberToString } from '@/utils/number';

export const TeamsControl = () => {
  const { teamPlansRequest, updatePlanActivity, addTeamPlan } = useTeamPlans();
  ...
  const handleAddTeam = useCallback(
    ({
      ...
      return addTeamPlan.mutateAsync({
        subscriptionCost,
        reward,
  ...
  return (
    <AdminSection title="Teams" isLoading={teamPlansRequest.isLoading}>
      <>
        ...
        <Box mt="16px" maxHeight="400px" overflowY="auto">
          {teamPlansRequest.data?.map((plan) => (
            <TeamInfo
              key={plan.teamPlanId.toNumber()}
              teamPlanId={plan.teamPlanId.toNumber()}
              reward={plan.reward}
              ...
              onActivate={() =>
                updatePlanActivity.mutateAsync({
                  planId: plan.teamPlanId.toNumber(),
                  isActive: true,
                })
              }
              onDeactivate={() =>
        ...
        {isOpen ? <AddTeamModal onClose={onClose} onSubmit={handleAddTeam} /> : null}
      </>
    </AdminSection>
  );
};

type TeamsControlProps = {
  teamPlanId: number;
  subscriptionCost: BigNumber;
  reward: BigNumber;
...
const TeamInfo: FC<TeamsControlProps> = ({
  teamPlanId,
  subscriptionCost,
  reward,
  stakingThreshold,
  ...
  return (
    ...
    >
      <Flex alignItems="center" mb="8px">
        <Text mr="12px">Team (id: {teamPlanId})</Text>
